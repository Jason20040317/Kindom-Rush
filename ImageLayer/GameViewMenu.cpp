...
bool GameViewMenu::init()
{
	...
	update->setPosition(Point(winSize.width - update->getContentSize().width / 2, update->getContentSize().height / 2 - 130));//商店和升级的图标位置
	shop->setPosition(Point(update->getPosition().x - 160, shop->getContentSize().height / 2 - 130));

	auto menu =Menu::create(shop,update,NULL);
...
void GameViewMenu::onEnterTransitionDidFinish()
{
	...
	starLabel->setString(String::createWithFormat("%d/18", UserDefault::getInstance()->getIntegerForKey(instance->SLOTX_STAR,0))->getCString());
