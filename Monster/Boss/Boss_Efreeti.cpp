...
Boss_Efreeti* Boss_Efreeti::createMonster(std::vector<Point> points,std::vector<std::vector<std::vector<Point>>> sonpath)
{
    ...
    if (monster && monster->init())
    {
		...
		monster->setMaxHp(6000);
		monster->setCurrHp(6000);
...
bool Boss_Efreeti::init()
{
	...
	addChild(baseSprite);
	cloud = Sprite::createWithSpriteFrameName("Boss_Efreeti_Cloud_0001.png");//boss的云雾效果
	cloud->setPosition(Point(baseSprite->getContentSize().width/2,baseSprite->getContentSize().height/2));
	baseSprite->addChild(cloud,-1);
	cloud->runAction(RepeatForever::create(Animate::create(AnimationCache::getInstance()->getAnimation(getName()+"cloud"))));//云雾动画
	createAndSetHpBar();
	lastState = stateNone;
	scheduleUpdate();
...
void Boss_Efreeti::addSons()//添加随从
{
...
void Boss_Efreeti::laugh()//boss笑声音效
{
