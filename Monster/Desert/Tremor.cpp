...
void Tremor::update(float dt)//与basemonster中同名函数不同的是没有无状态、冻结和死亡三种情况
{
	if(lastState!=getState()){//若状态更新
		switch (getState())//根据当前所处状态判断
		{
		case(stateAttackRight) : {
			setAttackByTower(true);
			hpBgSprite->setVisible(true);
			lastState = stateAttackRight;//默认向右边攻击
			stopMonsterAnimation();//停止之前动画
