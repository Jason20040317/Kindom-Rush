...
bool TowerPanleLayer::init()
{
	...
	circle = Circle::create();//根据射程显示塔的攻击范围
	addChild(circle);
	circle->setVisible(false);

	planesprite = Sprite::createWithSpriteFrameName("gui_ring.png");//铁环
...
void TowerPanleLayer::addTower(int type)//正式添加防御塔
{
...
void TowerPanleLayer::inAnimation()//在触摸层执行从小到大的动画
{
...
void TowerPanleLayer::addTempTower(int type)//作用是添加临时防御塔贴图，只有点击第二次时，才会正式建造防御塔
{
	if(tempTower!=NULL){
...
bool TowerPanleLayer::onTouchBegan(Touch *touch, Event *event)//添加4个按键，给四个按键添加触摸监听
{
