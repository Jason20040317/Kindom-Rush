...
void ::GameOption::initButton()
{
	...
	auto restart = Sprite::createWithSpriteFrameName("options_buttons_0003.png");//重新开始按键
	restart->setPosition(Point(-145,-100));
	restart->setTag(0);

	auto exit = Sprite::createWithSpriteFrameName("options_buttons_0001.png");//退出按键
	exit->setPosition(Point(145,-100));
	exit->setTag(1);
	auto music_bak = Sprite::createWithSpriteFrameName("options_butBg_0001.png");//继续按键
	...
	auto music = Sprite::createWithSpriteFrameName("options_optButtons_0001.png");//音乐图标
	...
	auto effect = Sprite::createWithSpriteFrameName("options_optButtons_0003.png");//音效喇叭图标
	...
	auto vibration = Sprite::createWithSpriteFrameName("options_optButtons_0005.png");//手机振动图标
...
void GameOption::onKeyReleased(EventKeyboard::KeyCode keycode, Event* event)
{
    switch (keycode)
    {
	case EventKeyboard::KeyCode::KEY_BACK:   //返回监听
		SoundManager::playClickEffect();
		static_cast<PlayerStateMenu*>(this->getParent())->removeGameOption();
