...
bool PlayerStateMenu::init()
{
	...
	//拓展的攻击功能：陨石，伞兵，锦囊 
	thunderStoneSprite = Sprite::createWithSpriteFrameName("power_portrait_fireball_0001.png");//陨石图标精灵
	...
	paratrooperSprite = Sprite::createWithSpriteFrameName("power_portrait_reinforcement_0001.png");//伞兵图标精灵
	...
	packSprite = Sprite::createWithSpriteFrameName("power_portrait_backpack_0001.png");//锦囊图标精灵
	...
	backPackSprite = Sprite::createWithSpriteFrameName("backPack_hover.png");//锦囊背景长条精灵
	...
	stoneTimer->setReverseDirection(true);//顺时针转动
	...
	stoneListener->onTouchBegan = [&](Touch* touch, Event* event){
		...
		Size size = target->getContentSize();
		Rect rect = Rect(0, 0, size.width, size.height);
		if(rect.containsPoint(locationInNode)){//若第一次点击
			if(completeThunder == true){//若冷却结束
				mTouchLayer->removeAllListener();//移除其他技能触摸监听
...
//陨石圆形倒计时进度条,添加定时器，更新ProgressTimer的状态，下文类似函数功能一样
...
void PlayerStateMenu::clearMonsters()//大炸弹技能
{
...
void PlayerStateMenu::shopSkill(int type)
{
	switch(type){
	case(0):{
		auto atomicBomb_plane = Sprite::createWithSpriteFrameName("atomicBomb_plane.png");//投放最大炸弹的飞机
		...
		atomicBomb_plane->runAction(Sequence::create(
			...
			,NULL));
		auto atomicBomb_bomb = Sprite::createWithSpriteFrameName("atomicBomb_bomb.png");//大炸弹
		...
		atomicBomb_bomb->runAction(Sequence::create(
			...
			CallFuncN::create(CC_CALLBACK_0(PlayerStateMenu::clearMonsters, this)),NULL));//消灭全部敌人
		}
		break;
	case(1)://大范围冰冻
		frozenMonsters();	
...
void PlayerStateMenu::frozenMonsters()//大范围冰冻在四个角生成冻结图片，通过一个精灵变化而来
{
	if(!isFrozen){
		//右上角
